...
action :install do
  unless current_resource.installed
    ...
    when 'mpkg', 'pkg'
      install_cmd = "installation_file=$(ls --color=never '/Volumes/#{volumes_dir}' | grep '.#{new_resource.type}$') && sudo installer -pkg \"/Volumes/#{volumes_dir}/$installation_file\" -target /"
